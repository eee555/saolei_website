...
def video_upload(request):
    ...
    # 查重
    collisions = list(VideoModel.objects.filter(timems=data["timems"], bv=data["bv"]).filter(path=data["path"]).filter(cl=data["cl"], op=data["op"], isl=data["isl"], video__identifier=data["identifier"]))
    if collisions:
