...
- `videomanager.tasks`：对于需要后台计算的自定义 `pluck`，由 `helper_video_pluck` 入队执行 `task_video_pluck`；任务完成后保存 `pluck`，再触发 `customranking`。
...
- `videomanager.tasks`：自定义录像的 `pluck` 通过后台任务计算；普通三级别在解析时直接计算。
...
- `VideoModel.update_redis()` 的职责已经不再包含队列写入/移除，但命名仍然过时。
  - 当前实际职责包括上传日志、录像数量统计、启动自定义 `pluck` 后台任务。
  - 后续建议拆成更明确的服务函数，例如“记录上传结果”“更新录像数量”“调度 pluck 任务”，再移除或重命名 `update_redis()`。
- `VideoModel.update_video_num()` 仍是模型方法，并直接修改 `UserMS` 统计字段。
  - 这是 `videomanager` 到 `msuser` 的耦合残留。
  - 如果继续解耦，应迁移到 `msuser` 的信号或服务中，并明确创建、删除、状态变化、模式/级别变化时的统计规则。
...
- `pluck` 计算分工需要保持清晰。
  ...
  - 大图自定义录像由后台任务计算，并通过保存 `pluck` 触发 `customranking`。
