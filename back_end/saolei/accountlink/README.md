...
1. 新增平台枚举与账号模型
   ...
   新模型需要加入 `PLATFORM_CONFIG`，并通过 Django 命令自动生成迁移脚本。

2. 新增 Mineracer 关联会话模型

   现有 `AccountLinkQueue` 只能表达 `platform + identifier + verified`，无法表达第三方确认中的临时状态。因此建议新增 `MineracerAccountLinkSession`，用于保存一次关联尝试：
   ...
   绑定成功后再写入 `AccountLinkQueue`，其中 `identifier` 保存 Mineracer `userId`，`verified=True`。
...
3. 新增 Ninja API
   ...
   - `POST /api/accountlink/mineracer/start/`
     ...
     - 保存本地会话，返回 `session_id`、`verification_uri_complete`、`expires_at`、`retry_after_ms`。
...
4. 绑定成功逻辑应放在数据库事务中，并锁定相关记录，避免并发重复绑定。
5. 过期会话不能继续绑定；如果 Mineracer 返回过期状态，本地也应标记为 expired。
6. 前端打开 Mineracer 链接时使用新窗口，并使用 `rel="noopener noreferrer"`。
7. 日志中不要记录 partner key，也不要完整记录可复用的 `deviceCode`。
...
2. 后端实现 `AccountMineracer`、关联会话模型、迁移和 Mineracer HTTP 客户端。
3. 后端实现 start/status API，并覆盖成功、过期、冲突、重复发起、会话越权等测试。
