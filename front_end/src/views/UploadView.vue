<template>
    ...
    <el-table :data="video_msgs" table-layout="auto">
        ...
        <el-table-column :label="$t('common.prop.action')" :width="130">
            <template #default="props">
                <el-button :disabled="!(['pass', 'designator', 'needApprove'].includes(props.row.status))"
                    @click="forceUpload(props.$index)"
                    :type="['pass', 'designator'].includes(props.row.status) ? 'success' : props.row.status == 'needApprove' ? 'warning' : 'info'"
                    circle><el-icon>
                        <Upload />
                    </el-icon></el-button>
