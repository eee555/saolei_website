# Introduction to Minesweeper Terminologies

## Fundamentals
`rTime` (real time) or `time`: the game starts from the first left click and ends at failing or finishing. The accuracy is 1ms on Open Minesweeper.

`Est Rtime` (Estimated Real Time), `estime` or `est`: 简写为estime或est。如果点到雷后游戏结束，假设接下来3BV/s不变，那么预计一共要用多少时间才能扫开本局。
